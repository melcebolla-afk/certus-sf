...
- **Search perf:** filtro marked/frequent se construye **una vez por nodo** (antes: probe+parse UCI por cada candidata → nps bajos / pérdidas por tiempo); match UCI con una sola `MoveList` (sin `to_move` × N).
