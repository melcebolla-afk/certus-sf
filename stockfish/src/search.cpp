    ...
    if (!rootNode)
    {
        ...
        if (threads.stop.load(std::memory_order_relaxed) || pos.is_draw(ss->ply)
            ...
            return (ss->ply >= MAX_PLY && !ss->inCheck)
                     ? (CERTUS_SET_EVAL_NEED(rootNode, PvNode, pos, ss), evaluate(pos))
                     : value_draw(nodes);
...
#ifdef CERTUS_SF
    return Certus::evaluate(pos, network[numaAccessToken], accumulatorStack, refreshTable,
...
void SearchManager::check_time(Search::Worker& worker) {
...
#ifdef CERTUS_SF
    // SF19 already allows stop before completedDepth>=1. Keep short-clock hard cap.
    ...
    if ((worker.limits.use_time_management() && (elapsed > hardLimit || stopOnPonderhit))
